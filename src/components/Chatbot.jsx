...
export default function Chatbot() {
  ...
  return (
    <>
      {/* Nút chat - fixed góc phải dưới */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="fixed bottom-5 right-5 z-50 w-14 h-14 rounded-full bg-[#d4a441] text-black shadow-lg hover:bg-[#c49431] transition-all flex items-center justify-center"
        aria-label="Mở chat"
      >
        <svg
      ...
      {/* Khung chat */}
      {isOpen && (
        <div className="fixed bottom-24 right-5 z-50 w-[380px] max-w-[calc(100vw-40px)] h-[480px] max-h-[70vh] bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl flex flex-col overflow-hidden">
          {/* Header */}
          <div className="px-4 py-3 bg-[#d4a441] text-black font-semibold flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-zinc-950">
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-zinc-800 border border-zinc-700 px-3 py-2 rounded-lg text-sm text-zinc-400">
                  Đang suy nghĩ...
                </div>
              </div>
            )}
